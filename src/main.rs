...
fn main() {
    ...
    let mut configuration_file = File::open("config.json").unwrap();
    // Read the file contents into a string
    let mut contents = String::new();
    configuration_file.read_to_string(&mut contents).unwrap();

    // Parse the JSON in order to extract the configuration parameters
    let configuration: Configuration = serde_json::from_str(&contents).unwrap();
    let WindowSize {
        width: window_width,
    ...
    let event_loop: EventLoop<_> = EventLoopBuilder::<RenderThreadEvent>::with_user_event()
        .build()
        .unwrap();
