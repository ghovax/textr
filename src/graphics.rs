...
use crate::{
    ...
    layouting::{layout_heading, layout_paragraph, FontStyles, BORDER_MARGIN},
};
...
impl GraphicSystem {
    pub fn new(
        ...
        log::debug!("Display and window successfully created");

        let program = Program::from_source(
            &display,
            include_str!("../shaders/glyphVertexShader.glsl"),
            include_str!("../shaders/glyphFragmentShader.glsl"),
            None,
        )
        .map_err(|error| {
            CustomError::with_source("Unable to create the program".into(), error.into())
        })?;
        log::debug!("Program for the glyphs rendering successfully created");
        ...
        log::debug!("Cache and the associated texture correctly initialized");
    ...
    pub fn draw_glyphs(
        ...
        for content in document.root.iter() {
            log::trace!("Layouting the element: {:?}", content);
        ...
        log::trace!("Finished layouting the document");
    ...
    pub fn set_window_title(&mut self, document_path: PathBuf) {
        ...
        log::debug!("Set the window title to {:?}", window_title);
    }
}
        ...
        match test_flag {
            ...
            TestFlag::CompareWithReferenceImages => {
                let failed_tests = similarity_scores
                    .iter()
                    .filter(|(_, similarity_score)| *similarity_score < Self::SIMILARITY_THRESHOLD)
                    .collect_vec();
