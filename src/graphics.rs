...
impl Texture {
    ...
    pub fn from_bitmap_data(
        ...
        // ...then instruct the way the texture should be sampled
    ...
    fn descriptor<'a>() -> wgpu::VertexBufferLayout<'a> {
        ...
        wgpu::VertexBufferLayout {
            ...
            attributes: &[
                ...
                // ...and this is specifying the texture coordinates, which are
                // offset by the vertex position attribute
                wgpu::VertexAttribute {
...
/// configuring and rendering to a window system the text.
pub struct RenderState {
    ...
    /// The cache for the textures, it allows the retrieval of a given texture
    /// by providing the associated character.
...
/// a given projection. If this is altered, then the shader needs to be changed as well.
#[repr(C)]
...
impl RenderState {
    /// Initialize a novel render state.
    pub async fn new(window: Window) -> Result<Self, anyhow::Error> {
        ...
        // Request the system for a GPU which is simply compatible with
        // rendering on a screen, as we need no special features for our
        // rendering purposes
        ...
        // one will result all the colors coming out darker. If you want to
        // support non sRGB surfaces, you'll need to account for that
        // when drawing to the frame.
        let surface_format =
            match surface_capabilities.formats.iter().copied().find(|format| format.is_srgb()) {
                Some(format) => format,
        ...
        // Configure the window surface for some rendering
        ...
        let configuration = wgpu::SurfaceConfiguration {
            ...
            present_mode: match surface_capabilities.present_modes.first() {
                Some(present_mode) => *present_mode,
                None => {
                    return Err(anyhow::anyhow!("No supported present mode found"));
                }
            },
            alpha_mode: match surface_capabilities.alpha_modes.first() {
                Some(alpha_mode) => *alpha_mode,
                None => {
                    return Err(anyhow::anyhow!("No supported alpha mode found"));
                }
            },
            view_formats: vec![],
        ...
        // Tell the GPU how it should treat the texture data
        ...
        // Configure the camera so that the text can be visible
        ...
        // Tell the GPU how it should expect the data to be uploaded onto it
        ...
        // Initialize the vertex buffer to an empty state
    ...
    /// Employ a pre-defined render-pass to display the text onto the surface of the window.
    pub fn render(&mut self) -> Result<(), anyhow::Error> {
        // Retrieve the surface where to write to
        ...
        // Draw the text onto the surface...
        {
            ...
            for (character_index, character) in self.text_characters.iter().enumerate() {
                ...
                render_pass
                    .draw(6 * (character_index as u32)..6 * (character_index as u32 + 1), 0..1);
            }
